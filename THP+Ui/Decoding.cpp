...
//################################## Read Process ###################################
    ...
    aux1.append(QByteArray::number(line.at(0), 2).rightJustified(8, '0').right(8)); //8 bits at the 1st byte
    aux2.append(aux1.right(5)); //the last 5 bits represents the 1st 5 bits of the tree size
    aux2.append(QByteArray::number(line.at(1), 2).rightJustified(8, '0').right(8)); //5 + 8 bits of the 2nd byte = 13 bits to tree size
    aux1.truncate(3); //getting the 1st 3 bits of the 1st byte to the trash size
